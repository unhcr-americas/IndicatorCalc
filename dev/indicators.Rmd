...
electricity <- function(.data, 
  ...
  varname <- purrr::map(data , names) |> 
             purrr::list_flatten(name_spec = "{inner}")
  #varname
  #varname <- purrr::map_df(data , names)
 
  ### Perform some  check..
  var_LIGHT01 <- varname[grepl(pattern = LIGHT01 , x = varname)]
  if(length(var_LIGHT01) == 0){
    cli::cli_alert_info("LIGHT01 not found in dataset")
    return(.data)
  }
  if(!all(0 %in% .data[[var_LIGHT01]] | 1 %in% .data[[var_LIGHT01]])){
    cli::cli_alert_info("The data does not include the expected values for var_LIGHT01")
    return(.data)
  }
  
  var_LIGHT03 <- varname[grepl(pattern = LIGHT03 , x = varname)]
  if(length(var_LIGHT03) == 0){
    cli::cli_alert_info("LIGHT03 not found in dataset")
    return(.data)
  }
  if(!all(0 %in% .data[[var_LIGHT03]] | 1 %in% .data[[var_LIGHT03]])){
    cli::cli_alert_info("The data does not include the expected values for var_LIGHT03")
    return(.data)
  }
  ...
  .data |>
  mutate(electricity=
           case_when(var_LIGHT01=="1" & 
                       (var_LIGHT03!="1" | 
                        var_LIGHT03!="96" |
                        var_LIGHT03!="98")  ~ "1", 
                     TRUE ~ "0" )
  ) |>
  mutate( electricity = labelled::labelled(electricity,
                                 labels = c(
                                   "Yes" = "1",
                                   "No" = "0"
                                 ),
...
main <- main |> 
  ...
  mutate(reachableU30=case_when( time_tot > 30 ~ "0", 
                              TRUE ~ "1") # reachable under 30 minutes
  ) |>  
  mutate(dwa_cond1=case_when(DWA01!="7" |DWA01 !="9" |DWA01 != "13" | DWA01 != "96" |DWA01 !="98" ~ "1",
                             TRUE ~ "0") # improved source
  ) |> 
  mutate(dwa_cond2=case_when( reachableU30=="1" & DWA02 == "3" ~ "1", #UNDER30 MIN
