...
#' @param .data object 
#' 
#' @param HEA01 variable name pattern used to encode the question: 
#'                    " In general, when anyone in your household is sick,
#'                     where do they go to seek care?"
#'                    
#' @param HEA03 variable name pattern used to encode the question: 
#'                    "How long does it take to go there when you use the mode
#'                     of transport that you mentioned above?"
#' 
#' @param expected_values values used to encode the question if not numeric
#'                       can be c(0,1) per default or c("yes","no")
#'                       
...
healthcare <- function(datalist, 
  ...
  check_map <- fct_check_map(datalist = datalist,
                             mapper = mapper)
  if (check_map == FALSE) {
    datalist <- fct_check_map(datalist = datalist,
                              mapper = mapper)
  }
...
main <- main |> 
  mutate(healthcare=
           dplyr::case_when((HEA01!="96" | HEA01!="98") & HEA03 <= 60 ~ "1",
                     TRUE ~ "0")
  ) |> 
...
```{r example-healthcare}
healthcare()
```
  
```{r tests-healthcare}
test_that("healthcare works", {
  expect_true(inherits(healthcare, "function")) 
})
```

## drinkingwater
    
```{r function-drinkingwater}
#' drinkingwater
#' 
...
#' @param .data object 
#' 
#' @param DWA01 variable name pattern used to encode the question: 
#'             "What is the main source of drinking water for this household?"
#'                    
#' @param DWA02 variable name pattern used to encode the question: 
#'             "DWhere is this source located?"
#'                    
#' @param DWA03 variable name pattern used to encode the question: 
#'        "How long does it take to go there, get water, and come back,
#'         including waiting time?"
#' 
#' @param expected_values values used to encode the question 
#'                       can be c(0,1) per default or c("yes","no")
#'                       
#' @param chain Boolean TRUE/FALSE to tell whether the function should return
#'               the dataset expanded with the variable or a single factor
...
#' 
#' @export
drinkingwater <- function(datalist, 
         mapper = list(
            hierarchy = "main",
            variablemap = data.frame(
              label = c("Does this household use anything for lighting?",
...
main <- main |> 
  mutate(time_DWA= dplyr::case_when(
...
main <- main |> 
  ...
  mutate(reachableU30= dplyr::case_when( time_tot > 30 ~ "0", 
                              TRUE ~ "1") # reachable under 30 minutes
  ) |>  
  mutate(dwa_cond1= dplyr::case_when(DWA01!="7" |DWA01 !="9" |DWA01 != "13" | DWA01 != "96" |DWA01 !="98" ~ "1",
                             TRUE ~ "0") # improved source
  ) |> 
  mutate(dwa_cond2= dplyr::case_when( reachableU30=="1" & DWA02 == "3" ~ "1", #UNDER30 MIN
                             reachableU30=="0" & DWA02 == "3" ~ "0", # NOT UNDER30 MIN
                             DWA02=="1" | DWA02=="2" ~ "1", # in the dwelling/yard/plot
                             TRUE ~ NA_character_ ) 
  ) |>  
  mutate(drinkingwater= dplyr::case_when(
...
```{r example-drinkingwater}
drinkingwater()
```
  
```{r tests-drinkingwater}
test_that("drinkingwater works", {
  expect_true(inherits(drinkingwater, "function")) 
})
```
...
#' @param .data object 
#' 
#' @param LIGHT01 variable name pattern used to encode the question: 
#'                    "Does this household use anything for lighting?"
#'                    
#' @param LIGHT03 variable name pattern used to encode the question: 
#'                    "What source of electricity is used most of the time in this household?"
#' 
#' @param expected_values values used to encode the question 
#'                       can be c(0,1) per default or c("yes","no")
...
main <- main |> 
  ...
  mutate(dwe01_cat= dplyr::case_when( #Only apartment and house
    (DWE01=="1" | DWE01=="2") ~ "1", TRUE ~ "0" )
  ) |> 
  mutate(dwe02_cat= dplyr::case_when( #unimproved floor when earth,sand,clay,mud, dung or other
    (DWE02=="1" | DWE02=="2" | DWE02=="96") ~ "0", TRUE ~ "1" )
  ) |> 
  mutate(dwe03_cat= dplyr::case_when( #unimproved roof all options except metal,wood,ceramic tiles, cement, roofing shingles/sheets
    (DWE03=="8" |DWE03=="9" | DWE03=="10" | DWE03=="11" |
       DWE03=="12" | DWE03=="13" ) ~ "1" , TRUE ~ "0")
  ) |> 
  mutate(dwe04_cat= dplyr::case_when( #improved wall: cement,stone,bricks,cement blocks, covered adobe, wood planks
...
main <- main |> 
  ...
  mutate(dwe05_cat= dplyr::case_when( ##if crowding <= 3, not overcrowded 
    crowding <= 3 ~ "1", TRUE ~ "0")
  )
...
main <- main |> 
  ...
  mutate(shelter= dplyr::case_when(
...
test_that("shelter works", {
  expect_true(inherits(shelter, "function")) 
})
```
...
impact2_2 <- function(datalist, 
  ...
  mutate(impact2_2= dplyr::case_when(
...
test_that("impact2_2 works", {
  expect_true(inherits(impact2_2, "function")) 
})
```
...
ind <- ind |>  #Those who could not access
  ...
  mutate(health_NOacc= dplyr::case_when(
...
ind <- ind |>   ## Add up who needed services ( both who accessed and did not access)
  ...
  mutate(impact2_3= dplyr::case_when(
...
ind<- ind |> 
  mutate(edu_primary= dplyr::case_when(
    EDU01==1 & EDU02==1 & EDU03==2 ~ 1, EDU01==0 | EDU02==0 ~ 0, TRUE ~ 0)
  ) |> 
  mutate(age_primary= dplyr::case_when(
...
ind <- ind |> 
  mutate(edu_secondary= dplyr::case_when(
    EDU01==1 & EDU02==1 & (EDU03==3 | EDU03==4) ~ 1, EDU01==0 | EDU02==0 ~ 0, 
    TRUE ~ 0)
  ) |> 
  mutate(age_secondary= dplyr::case_when(
...
impact3_3 <- function(datalist, 
  ...
  main <- main |> 
   mutate(impact3_3= dplyr::case_when(
     SAF01==1 | SAF01==2 ~ 1,
     SAF01==3 | SAF01==4 ~ 0 , 
...
outcome1_2 <- function(datalist, 
  ...
  ind <- ind |> 
    mutate(birthCertificate= dplyr::case_when(
  ...
  ind <- ind |> 
    mutate(birthRegistered= dplyr::case_when(
  ...
  ind <- ind |> 
    mutate(outcome1_2= dplyr::case_when(
...
#Make sure to delete REG05e below from the script if you don't have any stateless 
  ...
  ind <- ind |> 
    mutate(document_under5= dplyr::case_when(
...
#Make sure to delete REG01e below from the script if you don't have any stateless   
  ...
  ind <- ind |> 
    mutate(document_above5= dplyr::case_when(
  ...
  ##Combine both age groups
    ...
    mutate(outcome1_3= dplyr::case_when(
...
outcome4_1 <- function(datalist, 
  ...
  main <- main |> 
    mutate(outcome4_1= dplyr::case_when(
...
outcome4_2 <- function(datalist, 
 ...
 main <- main |> 
   mutate(outcome4_2= dplyr::case_when(
...
outcome5_2 <- function(datalist, 
  ...
  if (check_map == FALSE) {
    datalist <- fct_check_map(datalist = datalist,
     ...
     ind <- ind |> 
   mutate(outcome5_2= dplyr::case_when(
...
main <- main |> 
 mutate(dwe09_cat= dplyr::case_when( #affordable if HH pays rent and often and always without financial distress
...
main <- main |> 
  ...
  mutate(outcome9_1= dplyr::case_when(
...
main <- main |>  
  mutate(outcome9_2=
           dplyr::case_when(LIGHT01==1 & (LIGHT02==1 |LIGHT02==3 | LIGHT02==4 | LIGHT02==5 | LIGHT02==6 | LIGHT02==7 |LIGHT02==8) 
                      ~ 1, TRUE ~ 0)
  ) |> 
...
outcome10_1 <- function(datalist, 
  ...
  if (check_map == FALSE) {
    ...
    ind <- ind |> 
  mutate(outcome10_1= dplyr::case_when(
...
main <- main |> 
  mutate(outcome10_2= dplyr::case_when( 
...
outcome12_1 <- function(datalist, 
  ...
  mutate(outcome12_1= dplyr::case_when(
    (dwa_cond1=="1" & dwa_cond2=="1" ) ~ "1", TRUE ~ "0")
  ) |>  
...
main <- main |> 
  mutate(toi_cond1= dplyr::case_when(TOI01==1 |TOI01==2 | TOI01==3 | TOI01==4 |TOI01==5 |TOI01==6 | TOI01==7 | TOI01==9 ~ 1,
                             TOI01==8 | TOI01==10 | TOI01==11 | TOI01==12 | TOI01==96 ~ 0,
                             TRUE ~ NA_real_)
  ) |> 
  mutate(toi_cond2= dplyr::case_when(
  ...
  mutate(toi_cond3= dplyr::case_when(
...
### improved sanitation facility / Safe disposal in situ of excreta from on-site sanitation facilities / not shared with other HHs
  ...
  mutate(outcome12_2= dplyr::case_when(
...
outcome13_1 <- function(datalist, 
  ...
  ## So first we check that we have what we need in the data set
  #varname1 <- names(.data)
  #varname3 <- purrr::map(data , names) |>         purrr::list_flatten(name_spec = "{inner}")

  
  hierar <- "main"
  varname <- names(data[[hierar]])
  #varname
  #varname <- purrr::map_df(data , names)
 
  ### Perform some  check..
  var_BANK01 <- varname[grepl(pattern = "BANK01" , x = varname)]
  if(length(var_BANK01) == 0){
    cli::cli_alert_info("BANK01 not found in dataset")
    return(.data)
  }
  if(!all("no" %in% .data[[hierar]][[var_BANK01]] | "yes" %in% .data[[hierar]][[var_BANK01]])){
    cli::cli_alert_info("The data does not include the expected values for var_BANK01")
    return(.data)
  }
  
  var_BANK02 <- varname[grepl(pattern = "BANK02" , x = varname)]
  if(length(var_BANK02) == 0){
    cli::cli_alert_info("BANK02 not found in dataset")
    return(.data)
  }
  if(!all(0 %in% .data[[hierar]][[var_BANK02]] | 1 %in% .data[[hierar]][[var_BANK02]])){
    cli::cli_alert_info("The data does not include the expected values for var_BANK02")
    return(.data)
  }
    
  var_BANK03 <- varname[grepl(pattern = "BANK03" , x = varname)]
  if(length(var_BANK03) == 0){
    cli::cli_alert_info("BANK03 not found in dataset")
    return(.data)
  }
  if(!all(0 %in% .data[[hierar]][[var_BANK03]] | 1 %in% .data[[hierar]][[var_BANK03]])){
    cli::cli_alert_info("The data does not include the expected values for var_BANK03")
    return(.data)
  }
      
  var_BANK05 <- varname[grepl(pattern = "BANK05" , x = varname)]
  if(length(var_BANK05) == 0){
    cli::cli_alert_info("BANK05 not found in dataset")
    return(.data)
  }
  if(!all(0 %in% .data[[hierar]][[var_BANK05]] | 1 %in% .data[[hierar]][[var_BANK05]])){
    cli::cli_alert_info("The data does not include the expected values for var_BANK05")
    return(.data)
  }
...
outcome13_2 <- function(datalist, 
  ...
  if (check_map == FALSE) {
   ...
   main <- main |> 
  mutate(outcome13_2= dplyr::case_when(
    INC01==1 ~ 1,
...
main <- main |> 
  mutate(employed= dplyr::case_when(
    UNEM01==1 ~ 1,
    UNEM02==1 & UNEM07==3 ~ 1,
    UNEM04==1 ~ 1,
  ...
  mutate(unemployed= dplyr::case_when(
    employed==0 & UNEM09==1 & UNEM10==1 ~ 1,
    TRUE ~ 0)
  ) |> 
  mutate(labour_force= dplyr::case_when(
...
outcome14_1 <- function(datalist, 
  ...
  ##Combine both age groups
    ...
    mutate(outcome14_1= dplyr::case_when(
     (document_above5==1 | document_under5==1) ~ 1,  
     (document_above5==0 | document_under5==0) ~ 0)
    ) |> 
...
outcome16_2 <- function(datalist, 
  ...
  if (check_map == FALSE) {
    ...
    main <- main |> 
  mutate(outcome16_2= dplyr::case_when(
    SPF01a==1 |SPF01b==1 |SPF01c==1 |SPF01d==1 | 
      SPF01e==1 | SPF01f==1 | SPF01g==1 | SPF01h==1 ~ 1, 
    TRUE ~ 0)
  ) |> 
